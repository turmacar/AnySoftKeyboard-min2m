...
public class WordComposer implements KeyCodesProvider {
  ...
  /**
   * Holds the typed word as it appears in the input. Note: the length of this may be different than
   * the size of mCodes! But, the code-point length of this is the same as the size of mCodes.
   */
  ...
  /**
   * Swaps the first and second values in the codes array if the primary code is not the first value
   * in the array but the second. This happens when the preferred key is not the key that the user
   * released the finger on.
   *
  ...
  public void deleteTextAtCurrentPositionTillEnd(CharSequence typedTextToDeleteAtEnd) {
    ...
    if (mTypedWord.toString().endsWith(suffixToDelete)) {
      ...
      int codePointsToDelete = Character.codePointCount(suffixToDelete, 0, suffixToDelete.length());
